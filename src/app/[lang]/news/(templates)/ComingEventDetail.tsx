...
function ComingEventDetail(props:TypeProps, ref:React.ReactNode){
  ...
  return <Suspense fallback={null}>
    <div className={twMerge('', className)}>
      <motion.div className="fixed left-0 top-0 z-[99999] h-full w-full bg-golden-300"
      variants={{
        enter: {
          opacity: 1,
          transition: {
      ...
      animate="enter">
        ...
        <div className="absolute left-0 top-0 h-full w-full overflow-auto bg-golden-300 pb-10">
                    ...
                    hullList.map((node, index)=>{
                      return <div className="col-6 mb-5" key={index}>
                        <RatioArea className="mb-3" ratio="56.25">
                          <Image className="absolute left-0 top-0 h-full w-full object-cover" fill={true} src={node.hull?.exteriorImages?.[0]?.image?.node?.mediaItemUrl || ''} sizes="50vw" alt="" />
                        </RatioArea>
