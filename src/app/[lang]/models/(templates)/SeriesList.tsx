        ...
        allSeries?.nodes?.map((node, index:number)=>{
          return <div className="mb-10 w-1/2" key={index}>
            <RatioArea className="mb-4" ratio="63.47">
              <LinkWithLang className="absolute left-0 top-0 h-full w-full" href={`/models/${node.slug}`} lang={lang}>
                <Image className="absolute left-0 top-0 z-0 h-full w-full object-cover" fill={true} src={node?.yachtsSeriesCustomFields?.seriesKeyImage?.node?.mediaItemUrl || ''} alt="" />
              </LinkWithLang>
            </RatioArea>
